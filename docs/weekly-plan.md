...
## 第 1 周（2026/06/01 - 2026/06/07）：LLM API 和基础概念
...
### 周四（2026/06/04）：理解 temperature 和 streaming
...
### 周二（2026/06/09）：解析 tool args

- 概念：结构化参数、JSON schema、参数校验
- 实验：让模型生成 `read_file` 参数并在 Go 侧解析
- 记录：参数校验和越权路径风险

### 周三（2026/06/10）：实现 `read_file`

- 概念：tool result、错误返回、最小权限
- 实验：实现受控 `read_file`
- 记录：文件读取对应的系统安全对象

### 周四（2026/06/11）：实现 `write_file`

- 概念：写操作风险、覆盖、路径穿越
- 实验：实现受控 `write_file`
- 记录：写文件工具的防护点

### 周五（2026/06/12）：记录完整 tool calling 链路
...
## 第 3 周（2026/06/15 - 2026/06/21）：最小 Agent Loop
...
### 周二（2026/06/16）：加入 `run_shell_command`
...
### 周四（2026/06/18）：记录 tool call 运行细节

- 概念：输入、输出、错误、耗时、状态码
- 实验：输出结构化 tool call 日志
- 记录：审计字段初稿

### 周五（2026/06/19）：整理最小 Agent 样例
...
### 周二（2026/06/23）：实现最小检索

- 概念：retrieve、top-k、相关性
- 实验：用关键词检索代替复杂向量库
- 记录：检索结果为什么会污染 prompt

### 周三（2026/06/24）：把检索结果拼入 prompt
...
## 第 5 周（2026/06/29 - 2026/07/05）：Agent 攻击样例
...
## 第 8 周（2026/07/20 - 2026/07/26）：eBPF 文件观测

目标：采集文件访问和文件修改行为。

### 周一（2026/07/20）：采集 `openat`

- 概念：文件打开、路径、权限
- 实验：捕获 `openat` 事件
- 记录：文件读取和敏感路径

### 周二（2026/07/21）：测试 `openat` 路径语义

- 概念：相对路径、绝对路径、cwd、dfd
- 实验：测试 `openat` 在不同路径输入下的事件表现
- 记录：第一版路径解析限制；`openat2` 作为后续增强

### 周三（2026/07/22）：采集 `unlink`

- 概念：文件删除风险
- 实验：捕获删除事件
- 记录：破坏性操作审计

### 周四（2026/07/23）：采集 `rename`

- 概念：文件重命名、覆盖、规避
- 实验：捕获重命名事件
- 记录：修改类事件字段

### 周五（2026/07/24）：验证敏感路径访问
...
## 第 9 周（2026/07/27 - 2026/08/02）：eBPF 网络观测

目标：采集 Agent 触发的网络外联行为。

### 周一（2026/07/27）：选择网络采集点

- 概念：connect、目标 IP、端口
- 实验：确定采集字段
- 记录：网络事件设计

### 周二（2026/07/28）：采集 `connect`

- 概念：外联行为
- 实验：捕获 TCP connect 事件
- 记录：外联事件样例

### 周三（2026/07/29）：补充进程上下文

- 概念：网络事件归因
- 实验：关联 pid、comm
- 记录：外联和进程关系；`cgroup/netns/container context` 作为后续增强

### 周四（2026/07/30）：统一三类事件 JSON

- 概念：process/file/network event schema
- 实验：统一事件输出格式
- 记录：统一事件字段

### 周五（2026/07/31）：验证 shell 和 http 外联
...
## 第 11 周（2026/08/10 - 2026/08/16）：规则检测
...
### 周二（2026/08/18）：串联系统事件

- 概念：行为层证据
- 实验：把 exec/file/network 纳入同一 task timeline
- 记录：timeline 样例

### 周三（2026/08/19）：实现敏感文件意图规则

- 概念：prompt 请求敏感文件
- 实验：检测 prompt 层敏感意图
- 记录：意图规则样例

### 周四（2026/08/20）：实现实际访问规则
...
## 第 13 周（2026/08/24 - 2026/08/30）：行为基线轻量版

目标：补充研究深度，但不做复杂异常检测平台。

### 周一（2026/08/24）：定义正常任务集合

- 概念：benign task、baseline scope
- 实验：定义数学计算、文件处理、数据分析任务
- 记录：正常任务清单

### 周二（2026/08/25）：采集正常进程行为

- 概念：正常进程集合
- 实验：运行正常任务并采集 exec
- 记录：进程基线样例

### 周三（2026/08/26）：采集正常文件和网络行为

- 概念：正常文件路径、正常外联
- 实验：采集 file/network 行为
- 记录：文件和网络基线样例

### 周四（2026/08/27）：实现简单偏离检测

- 概念：rare process/path/destination
- 实验：检测偏离正常集合的行为
- 记录：偏离检测样例

### 周五（2026/08/28）：记录误报和漏报
...
## 第 15 周（2026/09/07 - 2026/09/13）：公开项目整理
...
### 周四（2026/09/10）：补充使用样例

- 概念：可复现性
- 实验：补充命令、输入、输出样例
- 记录：复现步骤

### 周五（2026/09/11）：项目预发布检查
...
### 周二（2026/09/15）：完成技术文章初稿

- 概念：问题、方法、实现、结果
- 实验：写完整初稿
- 记录：待补证据

### 周三（2026/09/16）：整理面试讲稿

- 概念：项目亮点、架构取舍、风险边界
- 实验：写 5 分钟项目讲解稿
- 记录：面试问答清单

### 周四（2026/09/17）：录制或彩排 Demo
